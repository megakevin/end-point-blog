...
2. It must avoid using the same term for different concepts. In conversation with humans, a lot of the meaning is extracted from the context. In software, not so much. Just like you can't have two classes with the same name within the same name space, the ubiquitous language cannot allow such ambiguities.
